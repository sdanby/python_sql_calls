...
def get_adjustment_fields_sql():
    return """
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF(coeff, 1) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF(coeff, 1) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS season_adj_time,
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS event_adj_time,
        ...
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF(coeff, 1) / NULLIF(age_ratio_male, 0) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF(coeff, 1) / NULLIF(age_ratio_male, 0) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS age_season_adj_time,
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF(coeff, 1) / NULLIF(age_ratio_sex, 0) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF(coeff, 1) / NULLIF(age_ratio_sex, 0) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS age_sex_season_adj_time,
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) / NULLIF(age_ratio_male, 0) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) / NULLIF(age_ratio_male, 0) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS age_event_adj_time,
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) / NULLIF(age_ratio_sex, 0) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) / NULLIF(age_ratio_sex, 0) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS age_sex_event_adj_time,
        (GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1) * (age_ratio_sex / NULLIF(age_ratio_male, 0)), 1) AS INTEGER), 0), :min_sec) / 60)::text || ':' || lpad((GREATEST(COALESCE(CAST(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1) * (age_ratio_sex / NULLIF(age_ratio_male, 0)), 1) AS INTEGER), 0), :min_sec) % 60)::text, 2, '0') AS sex_event_adj_time,
        ...
        time_seconds,
        GREATEST(COALESCE(time_seconds / NULLIF(coeff, 1), 0), :min_sec) AS season_adj_time_seconds,
        GREATEST(COALESCE(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1), 0), :min_sec) AS event_adj_time_seconds,
        ...
        GREATEST(COALESCE(time_seconds / NULLIF(coeff + COALESCE(coeff_event, 1) -1, 1) / NULLIF(age_ratio_male, 0), 0), :min_sec) AS age_event_adj_time_seconds,
        GREATEST(COALESCE(time_seconds / NULLIF(coeff + COALESCE(coeff_event, 1) -1, 1) / NULLIF(age_ratio_sex, 0), 0), :min_sec) AS age_sex_event_adj_time_seconds,
        GREATEST(COALESCE(time_seconds / NULLIF((coeff + COALESCE(coeff_event, 1) - 1), 1) * (age_ratio_sex / NULLIF(age_ratio_male, 0)), 0), :min_sec) AS sex_event_adj_time_seconds,
